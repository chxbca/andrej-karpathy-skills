---
name: clone-website
description: Reverse-engineer and clone one or more websites in one shot by extracting assets, CSS, content, interactions, responsive behavior, and component specs before building a pixel-accurate Next.js clone. Use when the user wants to clone, replicate, rebuild, reverse-engineer, migrate, or copy a website.
---

# Clone Website

Resolve the target URL or URLs from the user request. You are about to reverse-engineer and rebuild those targets as pixel-perfect clones.
...
The target is whatever page the user-provided URL resolves to. Clone exactly what's visible at that URL. Unless the user specifies otherwise, use these defaults:
...
2. Parse the user-provided target as one or more URLs. Normalize and validate each URL; if any are invalid, ask the user to correct them before proceeding. For each valid URL, verify it is accessible via your browser MCP tool.
...
**Moving layered backgrounds matter.** A hero/background that looks like one static image may actually be duplicated image tracks, parallax layers, canvas, video, or CSS/JS-driven transforms. Do not collapse it into one static background. Capture the same layer at `t=0` and again after 1–2 seconds, compare bounding boxes/computed transforms/pixels, and document whether the motion is time-driven, scroll-driven, or both.
...
- Background mountains/waves/clouds/stars or decorative hero layers that drift continuously over time
...
**Time sweep:** Wait on each visually rich section for at least 1–2 seconds without scrolling or clicking:
- Record any background or decorative layer that moves on its own (mountains, waves, clouds, stars, particles, carousels, gradient masks, canvas, video, Lottie).
- For duplicated image tracks, record the duplicated DOM nodes, their asset URLs, start/end x positions, speed/direction, layer order, and whether front/back layers move at different speeds.
- Verify movement with a measurable check, not memory: compare bounding boxes/computed `transform` values or capture two screenshots and pixel-diff the moving region.
- If the original moves, the clone must implement the same time-driven motion and include a validation assertion that the layer actually moves.
...
### Time-driven layered backgrounds (if applicable)
- **Layers:** <asset URL/path, DOM node count, z-index/order, dimensions>
- **Motion proof:** <t=0 position/transform/pixel sample> → <t=1–2s position/transform/pixel sample>
- **Motion model:** <duplicated image track | CSS keyframes | JS transform loop | video/canvas/Lottie>
- **Implementation approach:** <looped duplicate tracks, parallax transform, canvas recreation, video embed, etc.>
- **Reduced motion:** <how `prefers-reduced-motion` disables or simplifies it>
...
Based on complexity, dispatch builder agent(s) in worktree(s). Every builder receives the full spec inline, screenshot paths, shared imports, target file path, and a `npx tsc --noEmit` verification requirement.

### Step 4: Merge

As builder agents complete their work, merge their worktree branches into main and verify `npm run build` after each merge.

## Phase 4: Page Assembly

Wire sections together in `src/app/page.tsx`, implement page-level layout and behaviors, and verify `npm run build` passes clean.

## Phase 5: Visual QA Diff

After assembly, do NOT declare the clone complete.

1. Compare original and clone screenshots at desktop and mobile.
2. Test scrolling, clicks, hovers, tab switching, and animations.
3. For time-driven hero/background/decorative motion, capture or measure two frames 1–2 seconds apart on both original and clone. Do not rely on a single screenshot. The clone is incomplete if a moving original layer is static.
...
- [ ] For time-driven/background motion: duplicated tracks or animated layers are identified, measured at two timestamps, implemented, and validated with a movement assertion
...
- **Don't turn moving backgrounds into static images.** If mountains, waves, clouds, particles, star fields, gradients, or decorative tracks move over time, inspect the DOM over time and recreate the motion. A screenshot match at `t=0` is not proof.
...
- **Don't reference docs from builder prompts.**
...
When done, report total sections built, components created, spec files written, assets downloaded, build status, visual QA results, and known gaps.
